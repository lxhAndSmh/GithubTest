...
- 3.如果有冲突，会提示你调用git status查看冲突文件
- 4.解决冲突后，然后调用git add .和git commit -a -m""提交更改
- 5.放弃解决冲突，取消 merge，如果你最终决定放弃这次 merge，也需要执行一次 merge --abort,来手动取消它:git merge --abort

## 删除分支
- git branch -d<分支名> 如果该分支没有合并到主分支会报错(删除的本地仓库)
- git branch -D<分支名> 强制删除(删除的本地仓库)
- git push origin -d <remote/branch> 删除远程把远程仓库的branch

## 删除仓库中的文件夹(即忽略已经提交的文件)
...
> 在Git中，HEAD表示当前版本，就是最新提交的版本，即使用git log打印出来的位于第一位的版本，上一个版本就是HEAD^，上上个版本就是HEAD^^，当前向上100个可以写成HEAD~100。
...
2.如果命令行窗口关闭了，或者第二天后悔了，可以进入到该目录下，使用git reflog命令来查看以前的每一次命令，可以获得每次提交的commitid，就可以版本回退了。
$ git reflog   可以查看命令历史，包含提交的commit id

## 正确的提交流程
- 1.git stash 备份当前工作内容保存到git栈中
- 2.git pull <分支名> 更新代码
- 3.git stash pop 从git栈中读取最近一次保存的内容，回复工作区的相关内容,如果有冲突就提交解决冲突
- 4.git add . 添加所有的文件、文件夹
- 5.git commit -a -m"" 提交本次事务，即将add的文件提交到git仓库，引号内部表示本次提交的提示信息
...
## 忽略文件
忽略文件的原则：
- android Studio自动生成配置文件：不能上传到git上，否则的话，如果你的同事下载下来，但是它的studio(gradle)版本和你的不一样，或者其他配置的各种路径不一样，就需要重建项目，严重的话，根本无法重建项目，一片爆红！
